...
## Question 35
#### 1. PDF of a Uniform Distribution:
The PDF of a uniform random variable \( X \) on the interval $[-1, 1]$ is given by:
$\[
f(x) = \frac{1}{2}, \quad -1 \leq x \leq 1
\]$
This is a constant function over the interval.

#### 2. CDF of a Uniform Distribution:
...
#### 3. Sketch of the PDF and CDF:

- **PDF**: A horizontal line at $\( f(x) = \frac{1}{2} \)$ over $\( -1 \leq x \leq 1 \)$.
- **CDF**: A line starting at $\( (x = -1, F(x) = 0) \)$ and increasing linearly to $\( (x = 1, F(x) = 1) \)$.
